...
export default function DoctorBio() {
    return (
        <section id="doctor" className="py-24 bg-emerald-900 text-white relative overflow-hidden">
            ...
            <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 relative z-10">
                <div className="grid md:grid-cols-2 gap-16 items-center">
                    ...
                    <div className="order-2 md:order-1 space-y-8 animate-in slide-in-from-left duration-700">
                        <div>
                            <h3 className="text-emerald-400 font-bold uppercase tracking-wider text-sm mb-2">Meet Your Expert</h3>
                            <h2 className="text-4xl md:text-5xl font-extrabold mb-4">{doctorBio.name}</h2>
                            <p className="text-emerald-200 text-xl font-medium">{doctorBio.designation}</p>
                        </div>

                        <div className="flex gap-4">
                            <div className="bg-emerald-800/50 p-4 rounded-xl backdrop-blur-sm border border-emerald-700">
                                <GraduationCap className="text-emerald-300 mb-2" />
                                <p className="text-sm text-emerald-100">{doctorBio.qualification}</p>
                            </div>
                            <div className="bg-emerald-800/50 p-4 rounded-xl backdrop-blur-sm border border-emerald-700">
                                <Clock className="text-emerald-300 mb-2" />
                                <p className="text-sm text-emerald-100">{doctorBio.experience}</p>
                            </div>
                        </div>
                    ...
                    <div className="order-1 md:order-2 flex justify-center md:justify-end animate-in slide-in-from-right duration-700 relative">
                        <div className="relative w-full max-w-sm aspect-[3/4]">
                            <div className="absolute inset-0 border-2 border-emerald-500 rounded-2xl transform translate-x-4 translate-y-4"></div>
                            <div className="absolute inset-0 bg-emerald-800 rounded-2xl overflow-hidden shadow-2xl">
